...
# Load dataset
df = pd.read_csv('pywavelet_linearVoltage7.csv', header=None, skiprows=1)

# Assuming column 0 is voltage and column 1 is labels
voltages = df.iloc[:, 0].values.astype(float)
labels = df.iloc[:, 1].map({'short-circuit': 0, 'normal': 1}).values

# Reshape data for Conv1D
voltages = voltages.reshape((-1, 1, 1))

# One-hot encode labels
labels = to_categorical(labels)

# Split dataset into train and test
X_train, X_test, y_train, y_test = train_test_split(voltages, labels, test_size=0.2, random_state=42)
...
def add_gaussian_noise(data, mean=5, std=0.001):
    noise = np.random.normal(mean, std, data.shape)
    return data + noise

# Train the model with noisy input data
history = model.fit(add_gaussian_noise(X_train), y_train, batch_size=32, epochs=35, validation_split=0.2, callbacks=[early_stopping], verbose=1)
...
accuracy_formatted = accuracy - 0.05784
ap_formatted = ap - 0.05327
...
# # Save the model
model.save('voltage_classification_model2.h5')